...
const createRecordController = async (req, res, next) => {
    try{
        ...
        if(!name || !email || !age || !sex) return res.status(422).send("Insufficient details");
        const gallery = req.files.map(f=>f.buffer);
        const data =await createRecord({name, email, age, sex, gallery});
        res.send(data);
    }
    catch(e){
        next(e);
...
const updateRecordController = async (req, res, next) => {
    try{
        ...
        const data = await updateRecord(id,{name, email, age, sex, gallery});
